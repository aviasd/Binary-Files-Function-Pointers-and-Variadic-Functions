...
void readGarden(FILE* fp, Garden* pGarden) {
	...
	if (fileType == TEXT) {
		...
		fread(&binaryData, sizeof(char), 1, fp);//this include the type and the count' we have to fix it
...
void writeGarden(FILE* fp, const Garden* pGarden) {
	...
	if (fileType == TEXT) {
		...
		namelength = strlen(pGarden->name) + 1; //  numbers of bytes with additional of '\0'
